...
function update_script() {
  ...
  if check_for_gh_release "geoip-updater" "crazy-max/geoip-updater"; then
    ...
    create_backup /usr/local/bin/geoip-updater/data

    ARCH=$(arch_resolve x86_64 aarch64)
    CLEAN_INSTALL=1 fetch_and_deploy_gh_release "geoip-updater" "crazy-max/geoip-updater" "prebuild" "latest" "/usr/local/bin/geoip-updater" "geoip-updater_*_linux_${ARCH}.tar.gz"
